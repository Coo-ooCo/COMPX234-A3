    ...
    try:
        while True:
            ...
            try:
                ...
                break
            remaining_bytes = msg_size - 3
            if remaining_bytes < 0:
                break
            message_body = receive_n(client_socket, remaining_bytes)
            if not message_body:
                break
            ...
            response_with_length = f"{len(response):03d} {response}"
            client_socket.sendall(response_with_length.encode())
    ...
    with lock:
        if op == "R":
            ...
            if len(key) + 1 + len(value) > 970:
                increment_stat("error_count")
                return "ERR Key + value too long(max 970 chars)"
            if key in tuple_space:
                increment_stat("error_count")
